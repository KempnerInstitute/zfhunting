...
# Define experiment types and their properties
experiments = [
    {
        'name': 'food_speed',
    ...
    {
        'name': 'num_bots',
        'control_name': 'control_num_bots',
        'xlabel': 'Number of Bots',
        'col_idx': 4
    }
]

# Create the main figure with 3 rows and 5 columns
...
for exp in experiments:
    ...
    if len(exp_data) == 0:
        ...
        for row in range(3):
            ax = axes[row, col_idx]
            ax.set_xlabel(exp['xlabel'], fontsize=16)
            if col_idx == 0:
                ax.set_ylabel(row_labels[row], fontsize=16)
    ...
    ax1.set_xlabel(exp['xlabel'], fontsize=16)
    if col_idx == 0:  # Only leftmost plot gets y-label
        ax1.set_ylabel(row_labels[0], fontsize=16)
    ...
    ax2.set_xlabel(exp['xlabel'], fontsize=16)
    if col_idx == 0:  # Only leftmost plot gets y-label
        ax2.set_ylabel(row_labels[1], fontsize=16)
    ...
    ax3.set_xlabel(exp['xlabel'], fontsize=16)
    if col_idx == 0:  # Only leftmost plot gets y-label
        ax3.set_ylabel(row_labels[2], fontsize=16)
...
plot_path = os.path.join(plots_dir, "combined_episode_level_analysis.png")
